...
/*   Updated: 2024/08/25 12:45:21 by rbryento         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	exec_left_pipe(t_minishell *mini_data, t_tree *branch, int *pipe_fd)
{
	...
	close(pipe_fd[1]);
	execute_piped_commands(mini_data, branch->left);
	exit(EXIT_SUCCESS);
}
...
void	exec_right_pipe(t_minishell *mini_data, t_tree *branch, int *pipe_fd)
{
	...
	close(pipe_fd[0]);
	execute_piped_commands(mini_data, branch->right);
	exit(EXIT_SUCCESS);
}

void	leftright_pid(t_minishell *mini_data, t_tree *branch, t_pipepids *pipepids)
{
	pid_t	*pids;
	int		*pipe_fd;
